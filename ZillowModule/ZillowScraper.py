...
class ZillowScraper:
  ...
  def __init__(self, zipCodes:list[str], add_to_mongo=True):
    self.add_to_mongo = add_to_mongo
    self.ValidateZipcodes(zipCodes)
  ...
  def ParseListings(self, listings:list[dict], zipCode:str):
    ...
    for listing in listings:
      if listing['statusType'] == 'FOR_RENT':
        ...
        if self.add_to_mongo:
          self.AddToMongo(str(zipCode), data)
        else:
          listing_data.append(data)
    return listing_data

  def WriteListingToCSV(self,header:list,output_csv:str):
    output_str = ''
    for zipCode in self.zipCodes:
      print('getting:',zipCode)
      queryData = self.GetQueryData(zipCode)
      listings = self.GetListings(queryData)
      listings = self.ParseListings(listings, zipCode)
      if len(listings) == 0:
        continue
      df = pd.DataFrame(listings)
      df = df[header]
      output_str += df.to_csv(header=False,index=False).strip() + '\n'
